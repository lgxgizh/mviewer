...
int main(int argc, char **argv) {
  ...
  std::string csvPath = (argc > 1 && argv[1][0] != '-')
                            ? argv[1]
                            : std::string(MVIEWER_SOURCE_DIR) +
                                  "/build_msvc/bin/benchmark_results.csv";
  ...
  for (int i = 1; i < argc; ++i) {
    ...
    if (a == "--baseline" && i + 1 < argc)
      ...
      std::cout << "Usage: benchmark_scenario [--baseline <csv>] [--threshold "
                   "<ratio>] [--csv <path>]\n"
  ...
  if (!baselinePath.empty())
    std::cout << "Baseline: " << baselinePath << "  threshold: " << threshold
              << "x\n";
  ...
  if (!baselinePath.empty()) {
    ...
    for (const auto &c : cmp) {
      std::cout << (c.passed ? "PASS  " : "FAIL  ") << c.scenario
                << "::" << c.name << "  cur=" << c.current_avg_ms << "ms"
