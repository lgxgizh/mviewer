...
QString MainWindow::shortcutsHelpHtml()
{
    ...
    return QStringLiteral(
        ...
        "<tr><td><kbd>B</kbd> / <kbd>S</kbd> / <kbd>W</kbd> / <kbd>O</kbd> / "
        "<kbd>K</kbd></td><td>闪烁 / 分割 / 滑动 / 叠加 / 棋盘</td></tr>"
        "<tr><td><kbd>]</kbd> / <kbd>[</kbd></td>"
        "<td>] 差异阈值 +5 · [ 差异阈值 -5</td></tr>"
        "<tr><td><kbd>.</kbd> / <kbd>,</kbd></td>"
        "<td>. 叠加透明度 +5 · , 叠加透明度 -5</td></tr>"
        ...
        "侧栏</td></tr>"
        "<tr><td><kbd>1</kbd>~<kbd>8</kbd></td><td>两张及以下：N 联布局。超过两张且鼠标在窗格上：按住在该窗格临时看第 N 张；鼠标不在窗格上仍改布局。Ctrl+2/4/8 始终改布局</td></tr>"
        "<tr><td><kbd>P</kbd> 上一对 · <kbd>PgUp</kbd> 上一对 · "
        "<kbd>←</kbd> 上一对</td><td>比较上一对（连续对比）</td></tr>"
        "<tr><td><kbd>N</kbd> 下一对 · <kbd>PgDn</kbd> 下一对 · "
        "<kbd>→</kbd> 下一对</td><td>比较下一对（连续对比）</td></tr>"
        ...
        "<tr><td><kbd>Ctrl+Alt+A</kbd></td><td>批量分析导出</td></tr>"
        "</table>");
}
...
QString MainWindow::userGuideHtml()
{
    return QStringLiteral(
        ...
        "棋盘。叠加/棋盘的滑条用到才出现。</li>"
        "<li>勾选「显示差异」看热力图；「对齐」按整数像素平移配准后再算指标。</li>"
        "<li><kbd>P</kbd> 上一对 · <kbd>PgUp</kbd> 上一对 · <kbd>←</kbd> 上一对；"
        "<kbd>N</kbd> 下一对 · <kbd>PgDn</kbd> 下一对 · <kbd>→</kbd> 下一对。</li>"
        "<li>在图上<b>右键拖</b>画 ROI。每张图的选区旁显示该格 V 与 RGB 均值；完整表在检视面板。</li>"
        "<li><kbd>Esc</kbd>：有选区先清框，再按退出。也可点「退出比较」。</li>"
        "</ol>"
        "<h3>3. 比较窗口常用键</h3>"
        "<table border='1' cellpadding='4' cellspacing='0'>"
        "<tr><td><kbd>Z</kbd> / <kbd>D</kbd></td><td>同步缩放 / 同步拖动</td></tr>"
        "<tr><td><kbd>Space</kbd></td><td>恰好两张：按住时鼠标所在一侧不动，另一侧临时显示这一侧。超过两张改用数字键</td></tr>"
        ...
        "<tr><td><kbd>Ctrl+Shift+A</kbd></td><td>取消选择</td></tr>"
        "<tr><td><kbd>Ctrl+Alt+A</kbd></td><td>批量分析导出</td></tr>"
        "</table>"
