...
QPixmap ImageViewer::loadPixmap(const QString &path)
{
    ...
    if (it != m_cache.end()) {
        m_cacheOrder.erase(std::find(m_cacheOrder.begin(),
                                     m_cacheOrder.end(), path));
    ...
    for (int y = 0; y < h; ++y) {
        ...
        for (int x = 0; x < w; ++x) {
            ...
            const int lum = static_cast<int>(
                0.299 * qRed(c) + 0.587 * qGreen(c) + 0.114 * qBlue(c));
...
void ImageViewer::paintEvent(QPaintEvent *event)
{
    ...
    if (m_selectMode) {
        const QRect r = m_selecting
                            ? QRect(m_selStart, m_selEnd).normalized()
                            : selectedRegion();
    ...
    if (event->button() == Qt::LeftButton) {
        if (m_selecting) {
            ...
            if (r.width() > 5 && r.height() > 5) {
                QImage img = m_pixmap.toImage().convertToFormat(
                    QImage::Format_RGB32);
                const QRect imgRect = QRect(
                    static_cast<int>((r.x() - m_offset.x()) / m_scale),
                    static_cast<int>((r.y() - m_offset.y()) / m_scale),
                    static_cast<int>(r.width() / m_scale),
                    static_cast<int>(r.height() / m_scale))
                                         .normalized();
                const QRect valid = imgRect.intersected(
