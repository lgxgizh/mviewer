...
QStringList ImageViewer::listImages(const QString &dirPath) {
  ...
  QFileInfoList entries =
      dir.entryInfoList(kImageExtensions, QDir::Files, QDir::Name);
  QStringList result;
...
QPixmap ImageViewer::loadPixmap(const QString &path) {
  ...
  if (it != m_cache.end()) {
    m_cacheOrder.erase(
        std::find(m_cacheOrder.begin(), m_cacheOrder.end(), path));
  ...
  QPixmap pix;
  ImageData cached;
  if (CacheManager::instance().getMemory(CacheLevel::FullImage,
                                         path.toStdString(), cached)) {
  ...
  if (pix.isNull()) {
    ...
    if (!decoded.isNull()) {
      CacheManager::instance().put(CacheLevel::FullImage, path.toStdString(),
                                   decoded);
...
void ImageViewer::computeHistogram(const QPixmap &pixmap) {
  ...
  for (int y = 0; y < h; ++y) {
    ...
    for (int x = 0; x < w; ++x) {
      const QRgb c = line[x];
      const int lum = static_cast<int>(0.299 * qRed(c) + 0.587 * qGreen(c) +
                                       0.114 * qBlue(c));
...
void ImageViewer::paintEvent(QPaintEvent *event) {
  ...
  if (!m_pixmap.isNull()) {
    ...
    painter.drawPixmap(QRectF(m_offset, scaled), m_pixmap,
  ...
  if (m_selectMode) {
    const QRect r = m_selecting ? QRect(m_selStart, m_selEnd).normalized()
                                : selectedRegion();
    if (r.isValid() && (m_selecting || r.width() > 5) &&
        (m_selecting || r.height() > 5)) {
      painter.save();
