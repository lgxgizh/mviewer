...
void MetadataModel::rebuild()
{
    ...
    // ─── Image section ──────────────────────────────────────────────────
    {
        ...
        addLeaf(img, tr("位深"),
                m_meta.bitDepth > 0 ? QString("%1 位").arg(m_meta.bitDepth)
                                    : tr("未知"));
        addLeaf(img, tr("通道数"),
                m_meta.channels > 0 ? QString::number(m_meta.channels)
                                    : tr("未知"));
        addLeaf(img, tr("色彩空间"), colorSpaceText(toQString(m_meta.colorSpace)));
        if (m_meta.dpiX > 0 || m_meta.dpiY > 0)
        {
            addLeaf(img, tr("分辨率 (DPI)"),
                    QString("%1 × %2").arg(m_meta.dpiX).arg(m_meta.dpiY));
        }
        addLeaf(img, tr("方向"), orientationText(m_meta.orientation));
        addLeaf(img, tr("ICC 配置"),
                m_meta.hasIccProfile ? tr("已嵌入") : tr("无"));
    }
    ...
    if (m_raw.parsed)
    {
        ...
        if (m_raw.exposureSec > 0.0)
        {
            if (m_raw.exposureSec >= 1.0)
                addLeaf(raw, tr("曝光时间"),
                        QString("%1 s").arg(m_raw.exposureSec, 0, 'f', 2));
            else
                addLeaf(raw, tr("曝光时间"),
        ...
        {
            addLeaf(raw, tr("焦距"),
                    QString("%1 mm").arg(m_raw.focalLength, 0, 'f', 1));
        }
        if (m_raw.focalLength35mm > 0.0)
        {
            addLeaf(raw, tr("等效焦距"),
                    QString("%1 mm").arg(m_raw.focalLength35mm, 0, 'f', 1));
        }
        ...
        {
            addLeaf(raw, tr("原始尺寸"),
                    QString("%1 × %2").arg(m_raw.width).arg(m_raw.height));
        }
