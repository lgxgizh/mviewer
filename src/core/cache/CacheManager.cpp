...
CacheManager::CacheManager() { configure(m_config); }

void CacheManager::configure(const CacheConfig &cfg) {
  m_config = cfg;
  ImageCache::instance().setCapacity(ImageCache::Metadata,
                                     cfg.metadataCacheSize);
  ImageCache::instance().setCapacity(ImageCache::Thumbnail,
                                     cfg.thumbnailCacheSize);
...
void CacheManager::putMemory(CacheLevel level, const std::string &key,
                             const ImageData &img) {
...
bool CacheManager::getMemory(CacheLevel level, const std::string &key,
                             ImageData &out) {
  if (level == CacheLevel::Disk)
    return false;
...
bool CacheManager::get(CacheLevel level, const std::string &key,
                       ImageData &out) {
  if (level == CacheLevel::Disk) {
    if (getDisk(key, out)) {
      recordHit(level);
...
void CacheManager::put(CacheLevel level, const std::string &key,
                       const ImageData &img) {
  if (level == CacheLevel::Disk)
    putDisk(key, img);
  else
...
void CacheManager::clearMemory() { ImageCache::instance().clear(); }

void CacheManager::clearDisk() { DiskCache::instance().clear(); }
...
void CacheManager::putMetadata(const std::string &key,
...
bool CacheManager::getMetadata(const std::string &key,
                               mviewer::domain::ImageMetadata &out) const {
  std::lock_guard<std::mutex> lock(m_metaMutex);
  auto it = m_metaStore.find(key);
  if (it == m_metaStore.end())
    return false;
