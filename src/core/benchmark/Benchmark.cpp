...
void Benchmark::run(const std::string &name, std::function<void()> fn,
                    int iterations) {
  if (!fn || iterations <= 0)
    return;
  Result r;
...
void Benchmark::run(const std::string &name, std::function<void(int)> fn,
                    int iterations) {
...
void Benchmark::report() const {
  ...
  std::cout << std::left << std::setw(40) << "Name" << std::right
            ...
            << std::setw(10) << "Max(ms)" << std::setw(8) << "Iter"
            << std::endl;
  std::cout << std::string(78, '-') << std::endl;
  for (const auto &r : m_results) {
    std::cout << std::left << std::setw(40) << r.name << std::right
              << std::setw(10) << std::fixed << std::setprecision(3) << r.avgMs
...
void Benchmark::clear() { m_results.clear(); }
