...
std::string computeFileHash(const std::string &path, int64_t size,
                            int64_t mtime) {
  const QString key = QString::fromStdString(path) + QString::number(size) +
...
void ImageObject::computeHistogram() { m_frame.computeHistogram(); }
