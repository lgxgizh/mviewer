...
bool Encoder::encode(const ImageData &img, const std::string &path,
                     const Params &params) {
  if (img.isNull())
    return false;
...
std::vector<uint8_t> Encoder::encodeToBuffer(const ImageData &img,
  ...
  if (!writer.write(image))
    return {};
  return std::vector<uint8_t>(buffer.constData(),
