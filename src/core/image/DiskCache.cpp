...
void DiskCache::openDb() {
  ...
  if (!m_impl->db.open()) {
    qWarning() << "DiskCache: Failed to open DB:"
               << m_impl->db.lastError().text();
    m_enabled = false;
  }
}
...
bool DiskCache::get(const std::string &key, ImageData &out) {
  ...
  const size_t bytesToCopy =
      std::min(static_cast<size_t>(blob.size()), out.byteSize());
...
void DiskCache::put(const std::string &key, const ImageData &img) {
  ...
  q.addBindValue(
      QVariant::fromValue<qint64>(QDateTime::currentSecsSinceEpoch()));
  ...
  if (pruned)
    qDebug() << "DiskCache: pruned to" << entryCount() << "entries,"
             << totalBytes() << "bytes";
}
