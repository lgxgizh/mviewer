...
ImageFrame::ImageFrame(const mviewer::domain::ImageMetadata &meta,
                       const ImageData &pixels)
    : m_meta(meta), m_pixels(pixels) {}

/*static*/ ImageFrame ImageFrame::create(const std::string &path,
                                         const ImageData &pixels) {
  ...
  const std::string composite = path + "|" + std::to_string(meta.fileSize) +
...
ImageFrame::findAnalysis(const std::string &analyzer) const {
...
ImageFrame::findRenderCache(RenderCacheEntry::Tag tag) const {
