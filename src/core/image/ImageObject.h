...
public:
  ...
  const std::string &path() const { return m_frame.metadata().filePath; }
  const ImageData &image() const { return m_frame.pixels(); }
  ...
  ImageFrame &frame() { return m_frame; }
  const ImageFrame &frame() const { return m_frame; }
...
inline const int *ImageObject::histogram() const {
  return m_frame.hasHistogram() ? m_frame.histogram().luminance.data()
                                : nullptr;
}
