...
int main(int argc, char *argv[])
{
    ...
    // ─── 3. RGB24 toQImage & fromQImage ─────────────────────────────────────
    {
        ...
        CHECK(match, "RGB24 pixel byte match");

        // fromQImage with Format_RGB888
        ImageData back = mvcore::fromQImage(q);
        CHECK(!back.isNull(), "fromQImage(RGB888) non-null");
        CHECK(back.format == PixelFormat::RGB24, "fromQImage format RGB24");
        CHECK(std::memcmp(back.buffer->data(), src.buffer->data(),
                          static_cast<size_t>(w * h * 3)) == 0,
              "RGB888 fromQImage round-trip match");
    ...
    // ─── 4. BGRA32 toQImage & Fast Memcpy ───────────────────────────────────
    {
        ...
        CHECK(q.format() == QImage::Format_ARGB32, "BGRA32 format ARGB32");

        QRgb px = q.pixel(10, 10);
        CHECK(qRed(px) == 200, "BGRA32 Red channel match");
        CHECK(qGreen(px) == 120, "BGRA32 Green channel match");
        CHECK(qBlue(px) == 50, "BGRA32 Blue channel match");
        CHECK(qAlpha(px) == 180, "BGRA32 Alpha channel match");
    ...
    // ─── 5. RGBA32 toQImage SIMD & Channel Swap ─────────────────────────────
    {
