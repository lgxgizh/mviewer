...
ImageStats
AnalysisEngine::computeStatsROI(const ImageData &imgData,
                                const mviewer::domain::Selection &region) {
  ImageStats s;
  if (imgData.isNull())
    return s;
  const QImage image =
      mvcore::toQImage(imgData).convertToFormat(QImage::Format_RGB32);
...
ImageData AnalysisEngine::differenceMap(const ImageData &aData,
                                        const ImageData &bData) {
...
double AnalysisEngine::ssim(const ImageData &aData, const ImageData &bData) {
  QImage aa =
      mvcore::toQImage(aData).convertToFormat(QImage::Format_Grayscale8);
  QImage bb =
      mvcore::toQImage(bData).convertToFormat(QImage::Format_Grayscale8);
  const int w = std::min(aa.width(), bb.width());
  const int h = std::min(aa.height(), bb.height());
  if (w < 8 || h < 8)
    return 0.0;
...
double AnalysisEngine::noiseEstimate(const ImageData &imgData) {
  if (imgData.isNull())
    return 0.0;
  QImage img =
      mvcore::toQImage(imgData).convertToFormat(QImage::Format_Grayscale8);
...
ImageData AnalysisEngine::heatMap(const ImageData &grayData) {
  ...
  for (int y = 0; y < h; ++y) {
    ...
    for (int x = 0; x < w; ++x) {
      ...
      const int g =
          std::min(255, std::max(0, 255 - std::abs(v - 128) * 2)) + v / 2;
      const int b =
          std::min(255, std::max(0, 255 - v * 2)) + std::min(255, 255 - v);
      dst[x] = qRgb(std::min(255, r / 2), std::min(255, g / 2),
                    std::min(255, b / 2));
    }
  }
  return mvcore::fromQImage(out);
