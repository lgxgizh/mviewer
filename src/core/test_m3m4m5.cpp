...
static void testROIStats() {
  ...
  CHECK(outStats.pixelCount == 50 * 50,
        "out-of-bounds ROI clipped to 50x50 = 2500");
...
static void testEncoder() {
  ...
  CHECK(Encoder::formatForExtension("JPEG") == "jpeg",
        "formatForExtension(JPEG)==jpeg");
  CHECK(Encoder::formatForExtension("png") == "png",
        "formatForExtension(png)==png");
  CHECK(Encoder::formatForExtension("bmp") == "bmp",
        "formatForExtension(bmp)==bmp");
  CHECK(Encoder::formatForExtension("webp") == "webp",
        "formatForExtension(webp)==webp");
...
static void testCacheManager() {
  ...
  // 未命中
  ImageData miss;
  CHECK(!mgr.get(CacheLevel::FullImage, key, miss),
        "CacheManager miss returns false");
  ...
  // 命中
  ImageData hit;
  CHECK(mgr.get(CacheLevel::FullImage, key, hit),
        "CacheManager hit returns true");
...
static void testAnalyzerRegistry() {
  ...
  CHECK(reg.create("nonexistent") == nullptr,
        "create('nonexistent') returns null");
}
...
static void testTaskSchedulerDependency() {
  ...
  // toPriority mapping
  CHECK(TaskScheduler::toPriority(TaskScheduler::DecodePool) ==
...
static void testEntropy() {
  ...
  QImage flat = makeColorTest(100, 100, QColor(128, 128, 128));
  ImageFrame frameFlat =
      ImageFrame::create("/flat.png", mvcore::fromQImage(flat));
...
static void testRenderEngine() {
  ...
  CHECK(engine.overlayDifference(ImageData(), ImageData(), 0.5).isNull(),
        "overlay null ok");
  CHECK(engine.scaleRegion(ImageData(), {0, 0, 10, 10}, {5, 5}).isNull(),
        "scaleRegion null ok");
...
static void testCompareControllers() {
  ...
  CHECK(!heat.isNull() && heat.format == PixelFormat::RGB24,
        "DifferenceEngine heatmap");
}
...
static void testImageFrameExtras() {
  ...
  // Render cache
  ImageData thumb =
      mvcore::fromQImage(makeColorTest(32, 32, QColor(50, 50, 50)));
...
static void testRenderCommand() {
  ...
  CHECK(c1.interp == static_cast<int>(RenderInterp::Bilinear),
        "DrawImage interp");
  ...
  CHECK(c5.type == RenderCommandType::DrawPixelMarker,
        "DrawPixelMarker factory");
}
...
static void testAnalyzerCapabilityFramework() {
  ...
  CHECK(hasCapability(cap, AnalyzerCapability::SingleImage),
        "histogram has SingleImage");
  CHECK(!hasCapability(cap, AnalyzerCapability::GPU), "histogram no GPU");

  // Unknown analyzer returns None
  CHECK(reg.capabilitiesOf("nonexistent") == AnalyzerCapability::None,
        "unknown → None");

  // Capability bitwise ops work
  auto combined = AnalyzerCapability::SingleImage |
                  AnalyzerCapability::RegionOfInterest |
                  AnalyzerCapability::Streaming;
  CHECK(hasCapability(combined, AnalyzerCapability::SingleImage),
        "bitwise-or keeps SingleImage");
  CHECK(hasCapability(combined, AnalyzerCapability::Streaming),
        "bitwise-or keeps Streaming");
  CHECK(!hasCapability(combined, AnalyzerCapability::MultiImage),
        "bitwise-or not MultiImage");
}
