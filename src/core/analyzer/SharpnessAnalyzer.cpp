...
double SharpnessAnalyzer::computeSharpness(const ImageBuffer &v, int x0, int y0,
                                           int x1, int y1) const {
  ...
  for (int y = y0; y < y1; ++y) {
    for (int x = x0; x < x1; ++x) {
      auto lum = [&](int xx, int yy) {
        const uint8_t *p = v.data + static_cast<size_t>(yy) * v.stride() +
                           static_cast<size_t>(xx) * cpp;
...
bool SharpnessAnalyzer::analyzeRegion(
    const ImageFrame &frame, const mviewer::domain::Selection &region) {
