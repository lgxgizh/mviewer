...
// 0].
double NoiseAnalyzer::estimateLaplacian(const ImageBuffer &v, int x0, int y0,
                                        int x1, int y1) const {
  ...
  for (int y = y0; y < y1; ++y) {
    for (int x = x0; x < x1; ++x) {
      ...
      const double n4 =
          lum(x, y - 1) + lum(x, y + 1) + lum(x - 1, y) + lum(x + 1, y);
...
bool NoiseAnalyzer::analyzeRegion(const ImageFrame &frame,
