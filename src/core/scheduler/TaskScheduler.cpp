...
public:
  LambdaTask(TaskScheduler::TaskHandle ctx,
      ...
      : m_ctx(std::move(ctx)), m_work(std::move(work)),
...
TaskScheduler::~TaskScheduler() { delete m_impl; }
...
static bool
depDone(const std::unordered_map<uint64_t, TaskScheduler::TaskHandle> &handles,
        uint64_t id) {
...
TaskScheduler::submit(Priority prio,
                      std::function<void(const TaskContext &)> work,
                      std::vector<TaskId> deps, std::function<void()> done,
  ...
  auto *runnable = new LambdaTask(
      ctx, std::move(work), [this, done = std::move(done), ctx_id]() {
        if (done) {
...
void TaskScheduler::setQueueMaxThreads(Priority prio, int n) {
  m_impl->priorityQueues[static_cast<int>(prio)].setMaxThreadCount(
      std::max(1, n));
}
