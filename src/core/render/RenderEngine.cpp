...
QImage bilinearQ(const QImage &src, const QSize &target) {
  ...
  for (int y = 0; y < th; ++y) {
    ...
    const int y0 =
        std::max(0, std::min(sh - 2, static_cast<int>(std::floor(sy))));
    ...
    for (int x = 0; x < tw; ++x) {
      ...
      const int x0 =
          std::max(0, std::min(sw - 2, static_cast<int>(std::floor(sx))));
      const double fx = std::max(0.0, sx - std::floor(sx));
      double r = (1 - fx) * (1 - fy) * qRed(sl0[x0]) +
                 fx * (1 - fy) * qRed(sl0[x0 + 1]) +
                 (1 - fx) * fy * qRed(sl1[x0]) + fx * fy * qRed(sl1[x0 + 1]);
      double g = (1 - fx) * (1 - fy) * qGreen(sl0[x0]) +
                 fx * (1 - fy) * qGreen(sl0[x0 + 1]) +
                 (1 - fx) * fy * qGreen(sl1[x0]) +
                 fx * fy * qGreen(sl1[x0 + 1]);
      double b = (1 - fx) * (1 - fy) * qBlue(sl0[x0]) +
                 fx * (1 - fy) * qBlue(sl0[x0 + 1]) +
  ...
  for (int y = 0; y < th; ++y) {
    ...
    for (int x = 0; x < tw; ++x) {
      ...
      for (int m = -1; m <= 2; ++m) {
        ...
        const QRgb *sline =
            reinterpret_cast<const QRgb *>(src.constScanLine(syy));
        for (int n = -1; n <= 2; ++n) {
...
ImageData SoftwareRenderer::overlayDifference(const ImageData &base,
  ...
  for (int y = 0; y < h; ++y) {
    ...
    for (int x = 0; x < w; ++x) {
      ...
      line[x] = qRgb(std::clamp(r, 0, 255), std::clamp(g, 0, 255),
                     std::clamp(b, 0, 255));
    }
  }
...
ImageData SoftwareRenderer::scaleRegion(const ImageData &src,
  ...
  const QImage sub =
      full.copy(QRect(region.x, region.y, region.width, region.height));
...
ImageData RenderEngine::scaleStatic(const ImageData &src,
                                    const RenderSize &target,
                                    RenderInterp mode) {
  return instance().scale(src, target, mode);
}

ImageData RenderEngine::overlayDifferenceStatic(const ImageData &base,
                                                const ImageData &diff,
                                                double alpha) {
