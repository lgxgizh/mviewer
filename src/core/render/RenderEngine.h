...
public:
    ...
    ImageData scaleRegion(const ImageData& src, const RenderRect& region,
                          const RenderSize& target, RenderInterp mode) override;
};
...
struct RenderCommand {
    ...
    static RenderCommand drawImage(const ImageData& img, const RenderSize& tgt, RenderInterp m) {
        RenderCommand c; c.type = RenderCommandType::DrawImage;
        c.srcImage = img; c.targetSize = tgt; c.interp = static_cast<int>(m);
        return c;
    }
    static RenderCommand drawOverlay(const ImageData& img, double a) {
        RenderCommand c; c.type = RenderCommandType::DrawOverlay;
        c.overlayImage = img; c.alpha = a;
        return c;
    }
    static RenderCommand drawHistogram(const int* bins, int n, const RenderRect& r) {
        RenderCommand c; c.type = RenderCommandType::DrawHistogram;
        c.histCount = n; c.rect = r;
        for (int i = 0; i < n && i < 256; ++i) c.histData[i] = bins[i];
        return c;
    }
    static RenderCommand drawSelection(const RenderRect& r, int rgb) {
        RenderCommand c; c.type = RenderCommandType::DrawSelection;
        c.rect = r; c.rgba = rgb;
        return c;
    }
    static RenderCommand drawPixelMarker(int x, int y, int rgb) {
        RenderCommand c; c.type = RenderCommandType::DrawPixelMarker;
        c.targetSize = {x, y}; c.rgba = rgb;
        return c;
    }
};
