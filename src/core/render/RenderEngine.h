...
struct RenderCommand {
  ...
  static RenderCommand drawImage(const ImageData &img, const RenderSize &tgt,
                                 RenderInterp m) {
    RenderCommand c;
  ...
  static RenderCommand drawHistogram(const int *bins, int n,
                                     const RenderRect &r) {
    RenderCommand c;
...
public:
  ...
  static ImageData overlayDifferenceStatic(const ImageData &base,
                                           ...
                                           double alpha = 0.5);
  static ImageData
  scaleRegionStatic(const ImageData &src, const RenderRect &region,
                    const RenderSize &target,
                    RenderInterp mode = RenderInterp::Bilinear);
