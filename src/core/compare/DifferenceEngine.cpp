...
ImageData DifferenceEngine::heatMap(const ImageData& gray) {
    if (gray.isNull()) return ImageData();
    ...
    for (int y = 0; y < h; ++y) {
        ...
        for (int x = 0; x < w; ++x) {
            ...
            int r, gg, b;
            if (v < 128) { r = 0; gg = v * 2; b = 255 - v * 2; }
            else { r = (v - 128) * 2; gg = 255 - (v - 128) * 2; b = 0; }
            dst[x] = qRgb(std::clamp(r,0,255), std::clamp(gg,0,255), std::clamp(b,0,255));
        }
    }
    return mvcore::fromQImage(out);
}
