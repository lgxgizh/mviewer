...
struct PixelController
{
    ...
    std::vector<PixelSample> probe(const std::vector<ImageData>& frames,
        int imgX,
        int imgY) const
    {
        ...
        for (size_t i = 0; i < frames.size(); ++i)
        {
            ...
            if (!f.isNull() && imgX >= 0 && imgY >= 0 && imgX < f.width && imgY < f.height)
            {
                ...
                const uint8_t* p = v.data + static_cast<size_t>(imgY) * static_cast<size_t>(v.stride()) +
    ...
    ProbeResult inspect(const std::vector<ImageData>& frames, int imgX, int imgY, int baseIndex = 0) const
    {
        ProbeResult r;
