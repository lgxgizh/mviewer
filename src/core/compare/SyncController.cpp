...
SyncController::SyncController(CompareEngine& engine)
    : m_engine(engine)
{
}
...
bool SyncController::enabled() const
{
    return m_engine.syncEnabled();
}
double SyncController::scale() const
{
    return m_engine.syncTransform().scale;
}
