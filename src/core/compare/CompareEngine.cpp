...
CompareEngine::CompareEngine() { m_layout = CompareLayout::forCount(0); }
...
void CompareEngine::zoomAt(double viewX, double viewY, double factor,
                           int exceptIndex) {
  if (m_sync.enabled) {
    ...
    m_sync.scale = newScale;
             ...
             exceptIndex < static_cast<int>(m_cells.size())) {
...
void CompareEngine::fitCell(int index, const CellSize &viewport,
...
ImageData CompareEngine::differenceMap(int index, int baseIndex) {
  ...
  if (rawA.format != PixelFormat::RGB24 && rawA.format != PixelFormat::BGR24 &&
      rawA.format != PixelFormat::RGBA32 &&
      rawA.format != PixelFormat::BGRA32 &&
      rawA.format != PixelFormat::Grayscale8)
    return ImageData();
  if (rawB.format != PixelFormat::RGB24 && rawB.format != PixelFormat::BGR24 &&
      rawB.format != PixelFormat::RGBA32 &&
      rawB.format != PixelFormat::BGRA32 &&
  ...
  for (int y = 0; y < h; ++y) {
    const uint8_t *la =
        rawA.buffer.get() + static_cast<size_t>(y) * rawA.stride();
    const uint8_t *lb =
        rawB.buffer.get() + static_cast<size_t>(y) * rawB.stride();
    ...
    for (int x = 0; x < w; ++x) {
      ...
      dst[x] =
          static_cast<uint8_t>(dr + dg + db > 765 ? 255 : (dr + dg + db) / 3);
    }
  }
  return out;
}
