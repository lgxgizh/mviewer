...
{
    const QString path = QDir::tempPath() + "/mviewer_batch_" +
                         QString::fromStdString(name) + ".png";
...
int main(int argc, char *argv[])
{
    ...
    // ── Test 1: Resize + Export (PNG→PNG) ──────────────────────────
    {
        ...
        config.operations = {mviewer::domain::BatchOp::Resize,
                             mviewer::domain::BatchOp::Export};
        ...
        CHECK(result.fileResults.size() == 3,
              "Should process all 3 files");
        CHECK(result.totalSucceeded == 3,
              "All 3 files should succeed");
        CHECK(result.totalFailed == 0,
        ...
        for (const auto &r : result.fileResults)
        {
            ...
            CHECK(r.width <= 32 && r.height <= 32,
                  "Resized image should fit within 32x32");
    ...
    // ── Test 2: Progress callback ──────────────────────────────────
    {
        ...
        processor.setProgressCallback(
            [&progressCalls](int current, int total, const std::string &)
            {
        ...
        CHECK(progressCalls.front().first == 0,
              "First progress call should be (0, total)");
        CHECK(progressCalls.back().first == 2,
              "Last progress call should be (total, total)");
    }
    ...
    // ── Test 3: Rename pattern ─────────────────────────────────────
    {
        mviewer::domain::BatchJobConfig config;
        config.inputPaths = {p1.toStdString(), p2.toStdString()};
        config.operations = {mviewer::domain::BatchOp::Rename,
                             mviewer::domain::BatchOp::Export};
    ...
    // ── Test 5: Invalid file ───────────────────────────────────────
    {
        ...
        CHECK(!result.fileResults[0].errorMessage.empty(),
    ...
    // ── Test 6: Empty operations ───────────────────────────────────
    {
        ...
        CHECK(result.totalSucceeded == 1,
              "File with no operations should still 'succeed'");
    }
    ...
    // ── Test 7: Cancellation ──────────────────────────────────────
    {
        ...
        CHECK(result.fileResults.size() <= 2,
              "Cancellation should stop after at most 2 files");
