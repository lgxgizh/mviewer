...
void AnalysisPanel::reanalyze()
{
    ...
    // A pending materialization job means the current frame is not yet
    ...
    // Dual-image comparison is a built-in composite view, not a single registry
    // analyzer. Keep its synchronous behavior; invalidate any single-frame task
    // and clear its pending/result surface so a stale async delivery can never
    // overwrite the compare page.
    ...
    // Loaded single-frame path: a valid current ImageFrame with a materialized
    // image runs the selected analyzer (and any ROI fallback) on the Analysis
    // pool, latest-wins, and returns immediately — never on the UI thread, and
    // never re-materializing the image or recomputing the base stats/noise.
...
// a stale pending/manual text can never survive an explicit state replacement
// (setImage / setImages / builtin_compare). Path-keyed AnalyzerModel history is
// untouched — this only clears the local UI status.
