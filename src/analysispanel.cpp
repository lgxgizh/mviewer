...
void AnalysisPanel::setRegionStats(const QString &text)
{
    m_statsLabel->setText(QString("<h3>%1</h3><p>%2</p>")
            .arg(tr("Region Stats")).arg(text));
}

void AnalysisPanel::onAnalyzerSelected(int index)
{
    m_currentPluginIdx = index;
    if (index == 0) updateHistogramPage();
    else if (index == 1) updateComparePage();
    else updatePluginPage();
}
...
void AnalysisPanel::renderHistogramPixmap()
{
    ...
    auto drawChannel = [&bg, &p](const int *hist, const QColor &color) {
        ...
        for (int i = 0; i < drawBins; ++i) {
            ...
            for (int j = lo; j < hi && j < srcBins; ++j) sum += hist[j];
            agg[i] = sum;
...
void AnalysisPanel::updateComparePage()
{
    ...
    txt += QString("<table>"
            ...
            .arg(tr("Noise(A)")).arg(noiseLevelText(noiseA))
            .arg(tr("Noise(B)")).arg(noiseLevelText(noiseB));
...
{
    if (variance < 50) return tr("Very Low (%1)").arg(variance, 0, 'f', 1);
    if (variance < 150) return tr("Low (%1)").arg(variance, 0, 'f', 1);
    if (variance < 300) return tr("Medium (%1)").arg(variance, 0, 'f', 1);
    if (variance < 500) return tr("High (%1)").arg(variance, 0, 'f', 1);
