...
void ExportDialog::setupUi() {
  ...
  // 源文件信息
  mainLay->addWidget(
      new QLabel(tr("源文件: %1 张图片").arg(m_sourceImages.size())));
...
void ExportDialog::onBrowseOutput() {
  if (m_batchCheck->isChecked()) {
    ...
    QString path =
        QFileDialog::getSaveFileName(this, tr("保存为"), QString(), filter);
    if (!path.isEmpty())
...
void ExportDialog::exportBatch() {
  ...
  params.quality = m_qualitySpin->value();
  QString fmt =
      m_formatCombo->itemData(m_formatCombo->currentIndex()).toString();
  QString suffix = "." + fmt;
  ...
  for (int i = 0; i < m_sourceImages.size(); ++i) {
    ...
    if (!img.isNull()) {
      ...
      ImageData data =
          mvcore::fromQImage(img.convertToFormat(QImage::Format_RGB32));
      if (Encoder::encode(data, dst.toStdString(), params)) {
        ++success;
      } else {
        ++fail;
    ...
    m_statusLabel->setText(
        tr("Processing... %1/%2").arg(i + 1).arg(m_sourceImages.size()));
  }

  m_statusLabel->setText(
      tr("Done: success %1, fail %2").arg(success).arg(fail));
  if (fail == 0)
    accept();
}
