...
void PreviewPanel::rebuild()
{
    ...
    const double s =
        std::min(static_cast<double>(availW) / m_full.width(),
...
void PreviewPanel::paintEvent(QPaintEvent *event)
{
    ...
    painter.drawText(txtArea, Qt::AlignTop | Qt::AlignLeft,
                     name + "\n" + QString::number(m_imgW) + "×" +
                         QString::number(m_imgH) + "  " +
                         QString::number(m_fileSize / 1024) + " KB");
    painter.setPen(QColor(180, 180, 180));
    painter.drawText(
        txtArea.adjusted(0, 36, 0, 0), Qt::AlignTop | Qt::AlignLeft,
