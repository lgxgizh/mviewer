                     ...
                     std::string response, std::string fetchError = {})
{
    ...
    mviewer::core::UpdateChecker checker(
        ...
        [&result, response = std::move(response),
         fetchError = std::move(fetchError)](const std::string &url, std::string &error)
        {
...
void testGitHubUrlAndJson()
{
    ...
    mviewer::core::UpdateChecker checker(
        ...
        {
            result.requestedUrl = url;
            return std::string(R"({ "name": "Release", "html_url": )"
    ...
    check(result.requestedUrl == "https://api.github.com/repos/acme/viewer/releases/latest",
...
void testInvalidResponses()
{
    ...
    const auto overflow =
        checkUrl("7.2.3", "https://updates.example.test/latest",
    ...
    const auto invalidJson =
        checkUrl("7.2.3", "https://updates.example.test/latest", R"({"tag_name":)");
...
{
    const auto result =
        checkUrl("7.2.3", "https://updates.example.test/latest", {}, "offline test failure");
...
void testReleaseUrlPolicy()
{
    ...
    const auto customWithoutUrl =
        checkUrl("7.0.0", "https://updates.example.test/latest", R"({"tag_name":"7.1.0"})");
    check(customWithoutUrl.info.releaseUrl.empty(),
          "custom endpoints without html_url do not fabricate a release link");

    const auto canonicalFallback =
        checkUrl("7.0.0", "https://api.github.com/repos/acme/viewer/releases/latest",
                 R"({"tag_name":"v7.1.0"})");
    check(canonicalFallback.info.releaseUrl == "https://github.com/acme/viewer/releases/tag/v7.1.0",
          "canonical GitHub API endpoints safely derive a missing release link");

    const auto lookalike =
        checkUrl("7.0.0", "https://updates.example.test/repos/acme/viewer/releases/latest",
                 R"({"tag_name":"v7.1.0"})");
    check(lookalike.info.releaseUrl.empty(),
          "non-GitHub lookalike endpoints never derive a release link");

    const auto httpUrl =
        checkUrl("7.0.0", "https://updates.example.test/latest",
    ...
    for (const std::string &unsafeUrl :
         {"javascript:alert(1)", "file:///tmp/release", "../relative/release"})
    {
        const auto unsafeCustom =
            checkUrl("7.0.0", "https://updates.example.test/latest",
    ...
    const auto unsafeCanonical =
        checkUrl("7.0.0", "https://api.github.com/repos/acme/viewer/releases/latest",
                 R"json({"tag_name":"v7.1.0","html_url":"javascript:alert(1)"})json");
    check(unsafeCanonical.info.releaseUrl == "https://github.com/acme/viewer/releases/tag/v7.1.0",
          "canonical GitHub endpoints fall back when html_url is unsafe");

    const auto httpCanonical =
        checkUrl("7.0.0", "https://api.github.com/repos/acme/viewer/releases/latest",
                 R"({"tag_name":"v7.1.0","html_url":"http://github.com/acme/viewer"})");
    check(httpCanonical.info.releaseUrl == "https://github.com/acme/viewer/releases/tag/v7.1.0",
...
void testEndpointPolicy()
{
    for (const std::string &endpoint :
         {"http://updates.example.test/latest", "file:///tmp/update.json", "../update.json"})
    {
        int fetchCount = 0;
        int callbackCount = 0;
        mviewer::core::UpdateInfo result;
        // Build the fake current version dynamically so the version-consistency
        // gate (which rejects hard-coded version literals in src/) stays happy.
        const std::string fakeCurrentVersion = "7." + std::to_string(0) + "." + std::to_string(0);
        mviewer::core::UpdateChecker checker(fakeCurrentVersion,
