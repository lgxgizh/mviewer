// MViewer benchmark entry point — measures cache, encode, analysis, compare paths
#include <QImage>
...
static void benchDecode()
{
    ...
    Benchmark::instance().run("Decoder::decodeFull(1920x1080)", [tmpPath]() {
...
static void benchEncode()
{
    ...
    Benchmark::instance().run("Encoder::encode(JPEG 1920x1080)", [&data, tmpPath]() {
        Encoder::encode(data, tmpPath.toStdString());
    }, 20);
}
...
static void benchAnalysis()
{
    ...
    Benchmark::instance().run("AnalysisEngine::computeStats(1920x1080)", [&data]() {
        ImageStats s = AnalysisEngine::computeStats(data);
        (void)s;
    }, 30);

    Benchmark::instance().run("AnalysisEngine::noiseEstimate(1920x1080)", [&data]() {
...
static void benchPSNR()
{
    ...
    Benchmark::instance().run("AnalysisEngine::psnr(1920x1080)", [&a, &b]() {
        double v = AnalysisEngine::psnr(a, b);
        (void)v;
    }, 20);

    Benchmark::instance().run("AnalysisEngine::ssim(1920x1080)", [&a, &b]() {
...
static void benchCache()
{
    ...
    Benchmark::instance().run("ImageCache::put(FullImage)", [&data, &key]() {
        ImageCache::instance().put(ImageCache::Viewer, key, data);
    }, 100);

    Benchmark::instance().run("ImageCache::get(hit)", [&key]() {
        ImageData out;
        ImageCache::instance().get(ImageCache::Viewer, key, out);
        (void)out;
    }, 100);

    Benchmark::instance().run("CacheManager::get(Miss)", [&key]() {
        ImageData out;
...
static void benchROI()
{
    ...
    Benchmark::instance().run("AnalysisEngine::computeStatsROI(800x600)", [&data, &roi]() {
...
static void benchDifference()
{
    ...
    Benchmark::instance().run("AnalysisEngine::differenceMap(1920x1080)", [&a, &b]() {
...
int main(int argc, char** argv)
{
    ...
    std::cout << "  ImageCache totalUsedBytes = " << ImageCache::instance().totalUsedBytes() << std::endl;
    std::cout << "  CacheManager memoryUsageBytes = " << CacheManager::instance().memoryUsageBytes() << std::endl;
    return 0;
}
