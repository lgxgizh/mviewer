  ...
  case ThumbnailPanel::SortName:
    std::sort(entries.begin(), entries.end(),
              [](const QFileInfo &a, const QFileInfo &b) {
                return a.fileName().compare(b.fileName(), Qt::CaseInsensitive) <
                       0;
              });
    break;
...
QPixmap ThumbnailWorker::makeThumbnail(const QString &path) {
  ...
  if (full.width() > target || full.height() > target) {
    const double ratio =
        static_cast<double>(target) / std::max(full.width(), full.height());
  ...
  QPixmap scaled = QPixmap::fromImage(img).scaled(
      ThumbnailPanel::kThumbSize, ThumbnailPanel::kThumbSize,
...
ThumbnailPanel::~ThumbnailPanel() { stopWorker(); }
...
void ThumbnailPanel::resizeEvent(QResizeEvent *event) {
  ...
  if (m_compareBtn) {
    ...
    const int bh = 28;
    const int margin = 6;
    m_compareBtn->setGeometry(width() - bw - margin, height() - bh - margin, bw,
                              bh);
    m_compareBtn->raise();
  }
}
