...
private:
    UiTestResult compareOne(const UiTestFixture& f) {
        ...
        if (gold.isNull()) { r.message = "missing golden: " + f.goldenPath; return r; }
        if (curr.isNull()) { r.message = "missing current: " + f.currentPath; return r; }
        if (gold.size() != curr.size()) {
            r.message = "size mismatch: " + std::to_string(gold.width()) + "x" +
                        std::to_string(gold.height()) + " vs " +
                        std::to_string(curr.width()) + "x" + std::to_string(curr.height());
            return r;
        }
        ...
        for (int y = 0; y < h; ++y) {
            ...
            for (int x = 0; x < w; ++x) {
                ...
                if (dr > f.tolerance || dg > f.tolerance || db > f.tolerance) ++diffs;
            }
        }
        r.diffPixels = diffs;
...
static bool saveTestImages(const QString& dir, const std::vector<QImage>& imgs) {
    for (int i = 0; i < (int)imgs.size(); ++i) {
        ...
        if (!imgs[i].save(path)) return false;
    }
    return true;
}
