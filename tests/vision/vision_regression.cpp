...
//
// Usage:
//   ./vision_regression.exe --generate    (save golden images from current render)
...
public:
    ...
    void addTestCase(const std::string& name, double tolerance = 2.0,
                     double maxDiffFraction = 0.01) {
        m_tests.push_back({name,
            m_goldenDir + "/" + name + ".png",
            m_outputDir + "/" + name + ".png",
            tolerance, maxDiffFraction});
    }
...
private:
    VisionResult compareOne(const VisionTestCase& tc) {
        ...
        if (gold.isNull()) { r.message = "missing golden: " + tc.goldenPath; return r; }
        if (curr.isNull()) { r.message = "missing current: " + tc.currentPath; return r; }
        if (gold.size() != curr.size()) { r.message = "size mismatch"; return r; }
        ...
        for (int y = 0; y < h; ++y) {
            ...
            for (int x = 0; x < w; ++x) {
                ...
                if (dr > tc.tolerance || dg > tc.tolerance || db > tc.tolerance) ++diffs;
            }
        }
        r.diffPixels = diffs;
        r.diffFraction = static_cast<double>(diffs) / (w * h);
        r.passed = (r.diffFraction <= tc.maxDiffFraction);
        r.message = r.passed ? "OK" : "diff=" + std::to_string(diffs) + "px (" + std::to_string(int(r.diffFraction*100)) + "%)";
        return r;
    }
...
int main(int argc, char** argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (arg == "--compare") mode = "compare";
        else if (arg == "--generate") mode = "generate";
        else if (arg == "--golden-dir" && i+1 < argc) goldenDir = argv[++i];
