...
> **Historical record (2026-07-19); current state in `STATUS.md`.** Packaging and
> CI names below are as of that date: `pack_installer.ps1` / `pack_portable.ps1`
> were replaced by `scripts/package_release.ps1` / `scripts/package_portable.ps1`
> (M14.1), and the nightly quality docs are regenerated by
> `scripts/health_score.ps1` in the `publish-health` job — there is no `dashboard`
> job in `.github/workflows/nightly.yml`.
