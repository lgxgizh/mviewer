...
| M12 | Product Beta Hardening | 🚧 In progress (RFC `docs/acceptance/user_workflow.md` approved). Close real-user workflow gaps: (M12.1) user-workflow acceptance + Workspace ROI/analysis persistence; (M12.2) performance reality-check vs `performance.md` on `benchmark/data/` tiers; (M12.3) Windows installer deploying Qt plugins (qtiff/libtiff-6) so TIFF opens on clean Windows; (M12.4) tag→build→test→package release automation; (M12.5) Qt-boundary scan + thread-safety audit + `docs/api/`. No architecture changes — freeze from M11 holds. |
