...
| M47 | **Production-scale data path hardening** — source-backed display (LOD/tile) for Viewer/Compare, transactional async Workspace/Project restore, exact-source consumers kept full-resolution, large-image gates | In progress (started 2026-08-17). **Phase 0 complete**: deterministic large-image fixture corpus (`testdata/generate_large_fixtures.py` + `large_fixture_gate`), baseline recorder (`m47_large_image_baseline`), RFC `docs/rfc/M47_SOURCE_BACKED_DISPLAY.md`, baseline evidence in `docs/review/M47_PHASE0_BASELINE_2026-08-17.md` — incl. reproduction that 100 MP JPEG/TIFF currently cannot be opened (Qt 256 MB QImage allocation limit) and that every repaint of a 24 MP fit view stalls the UI thread 126–164 ms. |
