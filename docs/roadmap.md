...
| M16 | **Professional Compare** — Industry-tool compare workflow (P0) | ✅ Done. Sync Compare Engine: zoom/pan sync, focus-lock (n/1), cursor-sync crosshair (n/n), diff-overlay, multi-grid 2/3/4/8 — done; N-up layout presets (1~8) done; editing-within-compare done; reference/difference metrics (PSNR/SSIM + per-pane histogram overlay) done. |
| M17 | **Asset Management** — Rating, labeling, filtering, search, export (P1) | ✅ Done (M24-verified). Rating store (`RatingStore` + `ratingstore_tests`), unified filter + search, ExportManager (PNG/JPEG/TIFF/CSV/HTML/Report), Plugin Manager UI, Workspace layout restore, Analyzer workflow (right-click, batch, CSV/HTML/JSON export), auto-update checker. Remaining idea (not shipped): Color Label / Reject / Pick / Favorite as a unified tag taxonomy. |
| M18 | **AI Workflow** — Analyzer + AI + Embedding pipeline (P2) | ⬜ Planned. Caption generation, similarity search via embeddings, object detection, smart search; RAW demosaic (libraw decoder — RAW preview already ships via `RawDecoder`); GPU acceleration when 100MP/RAW/HDR becomes a bottleneck. |
...
| M42 | **Real-world resource boundedness & large-image Compare closure** — bounded Raw16/LOD memory, source/display separation, and RAW preview scan efficiency | Complete (2026-08-15); see `docs/review/M42_REALWORLD_RESOURCE_BOUNDEDNESS_2026-08-15.md`. |
...
| M44 | **Release readiness, data safety & structural convergence** — P0 file-operation safety, unified atomic exports, async persistence, architecture-boundary closure, complexity convergence, and release evidence | Automated RC ready (2026-08-15); final Release build and 88/88 CTest pass. Native Windows UX/ICC/DPI/multi-volume review remains manual pending. See `docs/review/M44_RELEASE_READINESS_STRUCTURAL_CONVERGENCE_2026-08-15.md`. |
| M45 | **Native Windows product qualification & desktop workflow convergence** — CommandStack callback safety, cancellable file transfers, clipboard paste lifecycle, delete semantics | Complete (2026-08-16); 90/90 full CTest gate. See `docs/review/M45_NATIVE_WINDOWS_PRODUCT_QUALIFICATION_2026-08-16.md`. |
| M46 | **Real-world workflow reliability & long-session release qualification** — async cancellation/delivery contract, Browse paint-path purity & latest-intent priority, crash-safe persistence, workflow soak convergence, architecture-gate honesty | Complete (2026-08-17); new `m46_repository/thumbnail/browse/persistence` suites + `m46_workflow_soak` + `architecture_gate_regression`; full gate re-run consecutively. Native hardware rows remain MANUAL/BLOCKED in `docs/review/M46_NATIVE_WINDOWS_QUALIFICATION_2026-08-17.md`; full evidence in `docs/review/M46_RELIABILITY_SOAK_2026-08-17.md`. |
| M47 | **Production-scale data path hardening** — source-backed display (LOD/tile) for Viewer/Compare, transactional async Workspace/Project restore, exact-source consumers kept full-resolution, large-image gates | In progress (started 2026-08-17). **Phase 0 complete** (fixture corpus, baseline recorder + reproduction that 100 MP JPEG/TIFF currently cannot be opened, RFC `M47_SOURCE_BACKED_DISPLAY.md`, evidence `M47_PHASE0_BASELINE_2026-08-17.md`). **Phase 1 complete** (`ISourceImageCapabilities` + `SourceImage` provider with native/fallback classification counters; QtDecoder capability implementation; `m47_source_tests` 8 groups). **Phase 2 complete** (Viewer LOD-first display: probe→LOD/region rasters over `SourceImage`, latest-intent/cancellation/lifetime-safe, 100 MP JPEG now displays with bounded RSS; `m47_viewer_lod_tests` 6 groups; full gate 100/100). **Phase 3 complete** (Compare source-backed display: infeasible sources (>60 MP, full RGB beyond Qt's 256 MB limit) skip the full load, keep their pane via a metadata-only placeholder frame so pane indices stay aligned, and display through bounded per-pane viewport LOD rasters — `SourceImage` native LOD for JPEG, honest blank fallback for TIFF; skips never count as load failures (duplicate paths included); zoom re-materializes at a denser edge with no full decode; `m47_compare_lod_tests` 6 groups; full gate 101/101). **Phase 4 complete** (exact-source consumers verified with source-backed panes present: the Pixel Inspector reports the exact full-resolution pixel for feasible panes and 无效 — never the display LOD — for infeasible ones; diff/PSNR/SSIM on an all-infeasible pair degrade to "—" with zero decodes; report export records placeholder pairs non-comparable without materializing the infeasible source, preserving requested pane order; `m47_exact_source_tests` 3 groups; full gate 102/102). **Phase 5 complete** (transactional async Workspace/Project restore: `openWorkspaceFile`/`openProjectFile` move the file read + JSON deserialize to a background worker, the UI applies the parsed document atomically for the current generation only — a newer open supersedes an in-flight one, read/parse failures report and leave the live session untouched, destroy mid-restore is safe; the duplicated inline project restore was removed in favor of the shared apply; `m47_restore_tests` 5 groups; full gate 103/103). **Phase 6 in progress** (deterministic large-image soak + benchmark evidence: `m47_large_soak` runs 6 rounds of 100MP Viewer/Compare display churn with zoom, A→B→A supersede, destroy mid-request, per-round pool drain, zero full-decode fallbacks, bounded + plateau-stable RSS and a converged scheduler graph; the `m47_large_image_baseline` recorder gained the post-Phase-2 100MP viewer LOD display section and a Compare source-backed section — 100 MP JPEG opens to display in ~0.5 s with +53 MB RSS (was: cannot open at all) and a 100 MP compare pair displays with +82 MB and zero full decodes). **Phase 7 complete** (release-evidence convergence: auto test matrix regenerated — 103 test sources incl. all M47 suites; RFC marked implemented through Phase 6; evidence review `docs/review/M47_SOURCE_BACKED_DISPLAY_2026-08-17.md`; STATUS updated). **Phase 8 complete** (final qualification: `.\build.ps1 Test` passed **104/104 three consecutive times**; architecture gate 0 violations; complexity strict gate 0 hard failures; native hardware rows remain MANUAL/BLOCKED per `docs/review/M46_NATIVE_WINDOWS_QUALIFICATION_2026-08-17.md`). **M47 complete.** |
...
| M52 | **Release Quality Truthfulness & Workflow Boundary Convergence** — PowerShell 5.1 UTF-8 truthfulness, honest complexity caps, source-backed Compare planning, MainWindow session split, and adversarial Browse/Compare boundaries | ✅ Automated closure complete (2026-08-28); current source-stable tree passed `build.ps1 Test` **114/114 twice consecutively**, with 0 complexity hard failures and 0 architecture violations. Advisory warnings remain visible accepted baseline debt; physical target-machine UX remains MANUAL/BLOCKED. See `docs/review/M52_RELEASE_QUALITY_WORKFLOW_CONVERGENCE_2026-08-28.md`. |
| M53 | **Large-Source Format Parity & Native Release Reality Closure** — bounded Windows TIFF display, streaming RAW preview scan, unified source/display failure reporting, large-source UI endurance, and release truth reconciliation | ✅ Automated closure complete (2026-08-29); `build.ps1 Test` passed 117/117 twice, architecture/complexity hard gates are green, and the 1.0.13 package contract passed. Native physical monitor/installer/UNC/network UX remains MANUAL/BLOCKED. See `docs/review/M53_LARGE_SOURCE_NATIVE_RELEASE_CLOSURE_2026-08-29.md`. |
| M54 | **Perceived Performance & Instant Browse** — real Browse E2E latency evidence, progressive large-directory discovery, cache lock/write-amplification reduction, viewport latest-wins, and foreground selected Preview | ✅ Automated closure complete (2026-08-29); M54 E2E benchmark and focused 23/23 regression set are green, and two consecutive source-stable `build.ps1 Test` gates passed 118/118 in 779.82 s and 782.14 s, with zero architecture violations and zero complexity hard failures. See `docs/review/M54_INSTANT_BROWSE_FINAL_2026-08-29.md`; new display-raster neighbor preload and physical perceived-UX rows remain MANUAL/follow-up. |
| M55 | **Interactive Latency, Memory & Navigation Closure** — bounded UI QPixmap working set, thumbnail demand ABA/overlap safety, exact scan/cache hot paths, source-backed LOD neighbor warmth, and coalesced TagStore persistence | ✅ Automated closure complete (2026-08-29); deterministic ABA/overlap and real-widget cache-bound tests are green, and two consecutive source-stable Release `build.ps1 Test` gates passed 119/119 in 749.06 s and 750.28 s with zero architecture violations and zero complexity hard failures. The standalone 10k/50k recorder is available; native fullscreen sequential-next and extended physical GUI smoothness remain MANUAL/BLOCKED. See `docs/review/M55_INTERACTIVE_LATENCY_MEMORY_NAVIGATION_CLOSURE_2026-08-29.md`. |
| M57 | **Multi-Frame / Multi-Page Image Architecture & Playback** — unified sequence capability, explicit frame identity, GIF/WebP playback, TIFF page navigation, bounded prefetch, Compare/analysis/persistence/export propagation, and Qt plugin truthfulness | ✅ Automated closure complete (2026-08-29); qgif/qwebp/qtiff fixture coverage and two source-stable Release `build.ps1 Test` gates passed **124/124** in 755.11 s and 753.71 s, with zero architecture violations and zero complexity hard failures. Physical native GUI feel, hostile animation corpus, 100MP multi-page TIFF RSS, and clean target-machine package qualification remain MANUAL/BLOCKED. See `docs/review/M57_PHASE0_MULTIFRAME_BASELINE_2026-08-29.md` and `docs/review/M57_MULTIFRAME_MULTIPAGE_CLOSURE_2026-08-29.md`. |
| M58 | **Large-directory query and search responsiveness** — value-semantics BrowseQuery, debounced cancellable latest-wins filtering/sorting, store snapshots, metadata batches, async SearchPanel, and 10k/50k deterministic acceptance | ✅ Automated closure complete (2026-09-01). Two source-stable `build.ps1 Test` runs passed 125/125; native GUI first-paint/typing cadence remains MANUAL/BLOCKED. See `docs/review/M58_PHASE0_LARGE_DIRECTORY_QUERY_BASELINE_2026-09-01.md` and `docs/review/M58_LARGE_DIRECTORY_QUERY_CLOSURE_2026-09-01.md`. |
| M59 | **Color-managed presentation contract** — canonical ICC/EXIF/bit-depth metadata, source→display target conversion, monitor-profile generation invalidation, CPU/GPU parity, and production Browse regression | ✅ Automated closure complete (2026-09-02); two source-stable Release `build.ps1 Test` runs passed **128/128** in 742.80 s and 743.22 s. GitHub Actions verification is recorded in `docs/review/M59_COLOR_MANAGED_PRESENTATION_CLOSURE_2026-09-02.md`; physical monitor LUT/HDR/mixed-DPI qualification remains MANUAL/BLOCKED. |
| M60 | **Linked ROI measurement** — canonical source-coordinate ROI, live linked overlay, asynchronous source RGB means/ratios/pixel counts, unequal-dimension guardrails, bounded source-region work, and Compare mode/session persistence | ✅ Automated closure complete (2026-09-02); two source-stable Release `build.ps1 Test` runs passed **129/129** in 820.73 s and 822.00 s. GitHub Actions packaging is recorded in `docs/review/M60_LINKED_ROI_CLOSURE_2026-09-02.md`; physical monitor/color-depth qualification boundaries remain documented. |
| M61 | **Professional linked ROI workflow convergence** — editable ROI across Grid/Split/Overlay/Swipe/Checkerboard, cancellable and backpressure-honest source measurement, compact HUD/TSV table, decoder region-truth audit, and real right-button UI coverage | ✅ Automated closure complete (2026-09-02); focused UI and 24/60/100 MP benchmark gates are green. Two source-stable Release `build.ps1 Test` runs passed **131/131** in 814.15 s and 795.36 s; native pointer feel and physical-monitor qualification remain MANUAL/BLOCKED. See `docs/review/M61_PROFESSIONAL_LINKED_ROI_CLOSURE_2026-09-02.md`. |
