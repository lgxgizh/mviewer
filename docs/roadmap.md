...
| M13 | **Product Beta** (public track: **Beta → 1.0 → 1.1 → 2.0**; old `v1.0.0-rc` = internal pre-release) | 🔵 In progress. Phase 1 (Product Workflow verification) done; Phase 2/3 (Dashboard + NSIS installer) done; review blockers (screenshot + large tier) closed; Phase 4 (real datasets) done; Phase 5 (Perfetto profiling) done; **Phase 6 (Plugin SDK stabilize) done**. Remaining: ⑦ GPU RFC ⑧ Public roadmap. |
