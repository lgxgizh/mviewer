...
| M13 | **Product Beta** (public track: **Beta → 1.0 → 1.1 → 2.0**; old `v1.0.0-rc` = internal pre-release) | 🔵 In progress — Phase 1 (Product Workflow verification) done. RFC `docs/rfc/M13_PRODUCT_BETA.md` approved 2026-07-19. Shift: stop building infrastructure, extract value. Subtraction rule applies to `core/`/`domain/` code. Phases: ① Workflow auto-verify (done) ② Benchmark Dashboard ③ Release Pipeline (NSIS .exe) ④ Real datasets ⑤ Perfetto profiling ⑥ Plugin SDK stabilize ⑦ GPU RFC ⑧ Public roadmap. Sequential; each gates on build+test green. |
